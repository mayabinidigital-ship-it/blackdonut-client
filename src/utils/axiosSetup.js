  ...
  axios.interceptors.response.use(
    ...
    error => {
      ...
      if (status === 401) {
        ...
        const isAuthCheck = 
          config.url.includes('/api/auth/user/profile') || 
          config.url.includes('/api/food-partner/me') ||
          config.url.includes('/api/auth/food-partner/me') ||
          config.url.includes('/api/chat/');
        
        // List of routes that MUST have login
        const protectedRoutes = ['/home', '/saved', '/food-partner/profile', '/user/profile', '/profile/user'];
        const isProtected = protectedRoutes.some(p => currentPath.startsWith(p));

        if (!isAuthCheck && isProtected && !window._authRedirectInProgress) {
          window._authRedirectInProgress = true;
          try {
            createNotification('Session expired. Please join in again!', 'error');
          } catch (e) {}
